...
int get_row_size(game_state s)
{
	...
	//returns an int of the row size variable attached to the game state struct
	return s->row_size;
}
...
int get_column_size(game_state s)
{
	...
	//returns an int of the column size variable attached to the game state struct
	return s->col_size;
...
int get_current_row(game_state s)
{
	...
	//returns the current row varaible within the current gamestates struct
...
int get_current_column(game_state s)
{
	...
	//returns the current column varaible within the current gamestates struct
...
bool valid(game_state s, int r, int c)
{
	...
	if (r <= s->row_size && c <= s->col_size) //if the variables r and c are within the boundaries for maximum column width and row size 
	{
		return true;
	}
	else
...
bool taken(game_state s, int r, int c)
{
	...
	//check wether a position is taken, minus the input by 1 for real world numbers
...
bool wall(game_state s, int r, int c)
{
	...
	//check wether a position is a wall minus the input by 1 for real world numbers
...
bool move(game_state s, int r, int c)
{
	...
	//check wether the positions the program wants to execute are taken or a wall
	if (!wall(s, r, c) && !taken(s, r, c))
	{
		set_current_col(s, c); //update current column for the game state s
		set_current_row(s, r);
		set_visited(s->board[r - 1][c - 1], true); //update wether the position has been visited
		return true;
	}
	else
	{
...
bool solved(game_state s)
{
	...
	//check wether the maze has been solved, -1 for real world values
